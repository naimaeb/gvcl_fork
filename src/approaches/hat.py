...
class Appr(object):
    ...
    def __init__(self, model,nepochs=100,sbatch=64,lr=0.05,lr_min=1e-4,lr_factor=3,lr_patience=5, clipgrad=10000,lamb=0.75,smax=400, args=None, **kwargs):
        self.model=model
    ...
    def eval(self,t,x,y, save_preds = False, dset = None):
        total_loss=0
        total_acc=0
        total_num=0
        self.model.eval()
        ...
        for i in range(0,len(r),self.sbatch):
            ...
            if save_preds:
                preds.append(torch.softmax(outputs[t], 1).detach().cpu().numpy())
        ...
        if save_preds:
            ...
            pickle.dump( preds, open( "{}_preds_{}.pkl".format(dset, t), "wb" ) )
            pickle.dump( labels, open( "{}_labels_{}.pkl".format(dset, t), "wb" ) )
